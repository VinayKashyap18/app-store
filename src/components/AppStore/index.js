...
import './index.css'
import AppI from '../AppItem'
import TabI from '../TabItem'
...
class AppStore extends Component {
  state = {input: '', active: tabsList[0].tabId}
  ...
  filter = searched => {
    ...
    const v = searched.filter(e => e.category === active)
    return v
  }
  ...
  search = () => {
    ...
    const {active} = this.state
    const k = appsList.filter(
      e =>
        e.appName.toLowerCase().inlcudes(input.toLowerCase()) &&
        e.category === active,
    )
    return k
  }
  ...
  render() {
    ...
    const filtered = this.filter(searched)
